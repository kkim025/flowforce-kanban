...
describe("UsersService", () => {
  ...
  describe("inviteUser", () => {
    it("should throw ConflictException if user already exists", async () => {
      ...
      await expect(service.inviteUser({ email: emailStr })).rejects.toThrow(ConflictException);
    });
    ...
    it("should create an invitation if user does not exist", async () => {
      ...
      (prisma.invitation.upsert as jest.Mock).mockResolvedValue({ email: emailStr, token: "token" });

      const result = await service.inviteUser({ email: emailStr, role: "ADMIN" });
      expect(result).toBeDefined();
  ...
  describe("removeUser", () => {
    ...
    it("should throw ConflictException when trying to delete the last admin", async () => {
      ...
      (repository.findAll as jest.Mock).mockResolvedValue([mockAdmin]);

      await expect(service.removeUser("admin-1")).rejects.toThrow(ConflictException);
    });
  ...
  describe("updateUserRole", () => {
    it("should throw NotFoundException if user does not exist", async () => {
      (repository.findById as jest.Mock).mockResolvedValue(null);
      await expect(service.updateUserRole("non-existent", "ADMIN")).rejects.toThrow(NotFoundException);
    });
    ...
    it("should throw ConflictException when trying to demote last admin", async () => {
      ...
      await expect(service.updateUserRole("admin-1", "MEMBER")).rejects.toThrow(ConflictException);
    });
    ...
    it("should allow demoting admin when there are other admins", async () => {
      ...
      const email2 = Email.create("admin2@test.com").getValue();
      const mockAdmin2 = User.create({
        email: email2,
        password: "hashedPassword",
        role: "ADMIN",
        status: "ACTIVE"
      ...
      (repository.findById as jest.Mock).mockResolvedValue(mockAdmin);
      (repository.findAll as jest.Mock).mockResolvedValue([mockAdmin, mockAdmin2]);
      (prisma.user.update as jest.Mock).mockResolvedValue({ id: "admin-1", role: "MEMBER" });
    ...
    it("should allow updating member to same role (no-op)", async () => {
      ...
      (repository.findById as jest.Mock).mockResolvedValue(mockUser);
      (repository.findAll as jest.Mock).mockResolvedValue([mockUser]);
      (prisma.user.update as jest.Mock).mockResolvedValue({ id: "user-1", role: "MEMBER" });
